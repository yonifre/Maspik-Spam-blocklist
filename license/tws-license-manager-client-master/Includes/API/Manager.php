...
final class Manager {
	...
	public function __construct( string $dirname, string $filename = '', string $parent_slug = '' ) {
      ...
      // maspik - yoni 
      	add_action( 'admin_init', array( $this, 'maspik_check_if_have_license' ) );
	...
	private function maybe_get_update( $id, $value, $meta ) {
		...
		$plugin->plugin      = $id;
		$plugin->url         = esc_url( "{$this->server_url}/{$this->product_slug}" ); // yoni
...
<a class="dashboard" href="<?php echo esc_url_raw( add_query_arg( array( 'referrer' => $this->page_slug ), admin_url() ) ); ?>">← <?php esc_html_e( 'Dashboard', 'tws-license-manager-client' ); ?></a> */;?>
  ...
  public function run_scheduled_task() {
	...
	public function terminate_scheduled_task() {
  	...
  	public function maspik_check_if_have_license() {
      $is_supporting = $this->get_license('status') == "active" ? 1 : 0 ;
      if( get_option( "maspik_is_supporting" ) ) {
        update_option( "maspik_is_supporting", $is_supporting); 
      }else{
        add_option( "maspik_is_supporting", $is_supporting );
      }
      update_option( "maspik_is_supporting", $is_supporting );
    }
